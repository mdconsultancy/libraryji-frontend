...
- [x] C1. Migration: add `permissions` JSON column to `tenant_user` (existing staff rows backfilled with full access to all 4 modules — no regression for anyone already in the system).
- [x] C2. Backend: `EnsurePermission` middleware (module, action) — full pass for admin/super_admin, pivot-lookup check for staff; registered as `permission` alias.
- [x] C3. Backend: split the `halls`/`members`/`payments` `apiResource` routes (and `tenant-settings` show/update, now reachable by staff too instead of admin-only) into explicit per-verb routes carrying `permission:module,action`; `StaffController::store`/`update` accept + persist a `permissions` object into the `tenant_user` pivot row alongside `role`. Also found & fixed along the way: `users.email` was `NOT NULL` in the schema, so "email optional" needed a real migration, not just relaxed validation (`2026_08_08_052343_make_users_email_nullable`).
- [x] C4. Backend: `User::tenants()` pivot load includes `permissions` (`withPivot('role','permissions')`); `StaffController::withCurrentTenantPivot()` surfaces both on show/store/update responses for the edit form to prefill.
- [x] C5. Frontend: `usePermission(module, action)` hook; Staff form reworked — Library select (required, hidden/locked when only one), Name (required), Email (optional), Phone (optional), Password (required) + Generate Password button, and a permission matrix (4 modules x up to 4 actions, "—" where an action doesn't apply e.g. Library add/delete) as checkboxes, shown only when role=staff.
- [x] C6. Frontend: gated Add/Edit/Delete buttons and page-level view access (`usePermissionGuard`, redirects to `/` if denied) on Halls, Members, Payments, Library(Settings) pages. Also found & fixed along the way: `Sidebar.tsx` had a second, completely hardcoded logo (`FullLogo` was imported but commented out!) — replaced with `FullLogo`, and sidebar items for the 4 gated modules now also check `usePermission(module,'view')` so staff without view access don't see a dead link.
- [x] C7. Verified over **real HTTP** (not just logic-level): started `php artisan serve`, created a committed (then cleaned up) admin+staff+Library with staff permissions `{halls: view only, members: view+add, payments: none, library: none}`. Confirmed exactly the expected mix of 200/403 across GET/POST on halls/members/payments/tenant-settings, staff 403 on `/auth/select-library` regardless of body, and admin bypassing every check.
...
- [x] D1. Migration + model: add `max_libraries` (nullable int, null = unlimited, default 1 so existing plans don't silently change behavior) to `subscription_plans`; `SubscriptionPlan` fillable, the super-admin Subscription Plans create/edit form + table column, and the frontend type.
- [x] D2. Backend: `GET /admin/libraries-summary` (admin-only) -> `{used, limit, remaining, exceeded}` — `used` = count of Libraries the admin belongs to (role=admin, any status), `limit` = MAX(max_libraries) over their active subscriptions (0 with no active subscription at all, null/unlimited if any active plan has no cap).
- [x] D3. Backend: `POST /admin/libraries` (`LibraryController`) — creates an *additional* Library for the already-authenticated admin (reuses the account; does NOT create a new User, unlike `/auth/register`), 422s if at limit, attaches `tenant_user` (role=admin), sets it as the new `current_tenant_id`. New Library starts `suspended` (payment-pending), same as registration. Requires its own email/phone (found along the way: `tenants.email` is globally unique NOT NULL, so a second Library can't just inherit the admin's own contact email — their first Library's tenant row already has it).
- [x] D4. Frontend: header "Libraries X/Y" indicator (admin-only, next to the Library Switcher) — a "+" button opening a "New Library" dialog (posts to `/admin/libraries`, refreshes the auth user, then hard-navigates to `/select-plan` to complete first payment for it — same reasoning as the switcher: SWR's cache isn't per-tenant) when under limit, or an "Upgrade Plan" link to the existing `/select-plan?upgrade=1` flow when at limit.
- [x] D5. Verified: tinker-reproduced `computeSummary()` end to end — no active subscription (limit 0, exceeded), subscribing to a max_libraries=1 plan (still exceeded at used=1), upgrading to a max_libraries=2 plan via a second active TenantSubscription row (exceeded flips to false, remaining=1) — confirming the upgrade propagates automatically with no extra step, then adding a second Library (back to exceeded). Super Admin User Management already surfaces `activeSubscription.plan` per Library from Phase 1, so `max_libraries` is visible there for free.
